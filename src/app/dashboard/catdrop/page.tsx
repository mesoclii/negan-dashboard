...
export default function CatDropEnginePage() {
  ...
  const [cfg, setCfg] = useState<CatDropCfg>(EMPTY);
  ...
  async function save() {
    ...
    try {
      const r = await fetch("/api/setup/games-config", {
        ...
        body: JSON.stringify({ guildId, patch: { catDrop: cfg } }),
      });
      const j = await r.json().catch(() => ({}));
      if (!r.ok || j?.success === false) throw new Error(j?.error || "Save failed");
      setCfg({ ...EMPTY, ...(j?.config?.catDrop || cfg) });
      setMsg("Cat Drop engine config saved.");
    } catch (e: any) {
        ...
        <div style={{ display: "grid", gap: 12 }}>
          <section style={box}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 10 }}>
              ...
              <div><div>Rare Cat Chance (%)</div><input style={input} type="number" value={cfg.rareCatChancePercent} onChange={(e) => setCfg((p) => ({ ...p, rareCatChancePercent: Number(e.target.value || 0) }))} /></div>
