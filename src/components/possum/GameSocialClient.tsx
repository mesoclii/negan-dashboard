      ...
      {!loading ? (
        <>
          <div style={card}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(260px,1fr))", gap: 12 }}>
              ...
              <div style={card}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                  <button type="button" style={button} disabled={saving} onClick={() => void runAction("gameIdentity", "confirmVerification", verificationForm, "Verification confirmed.")}>Confirm Proof</button>
          ...
          <div style={card}>
            ...
            <div style={{ color: "#ffb7b7", fontSize: 12, marginBottom: 12 }}>Manual snapshots still work, but Apex, Fortnite, and CoD can now use real legal-safe adapter endpoints with rate limits, cooldowns, failover, verification proof checks, and scheduled sync.</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(160px,1fr))", gap: 10 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(260px,1fr))", gap: 12, marginTop: 12 }}>
              <div style={card}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                  <button type="button" style={button} disabled={saving} onClick={() => void runAction("gameProvider", "upsertStats", { ...statsForm, rating: parseOptionalNumber(statsForm.rating), kills: parseOptionalNumber(statsForm.kills), wins: parseOptionalNumber(statsForm.wins), kd: parseOptionalNumber(statsForm.kd), matches: parseOptionalNumber(statsForm.matches) }, "Saved provider stats snapshot.")}>Save Stats</button>
                  <button type="button" style={button} disabled={saving} onClick={() => void runAction("gameProvider", "refreshProviderUser", refreshForm, "Requested external provider refresh for that member.")}>Refresh From Adapter</button>
                </div>
              </div>
              <div style={card}>
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  ...
                  <input style={input} placeholder="Adapter refresh user ID" value={refreshForm.userId} onChange={(event) => setRefreshForm((prev) => ({ ...prev, userId: event.target.value }))} />
          ...
          <div style={card}><h3 style={sectionTitle}>Playtime</h3><div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(160px,1fr))", gap: 10 }}><label><input type="checkbox" checked={Boolean(getValue(engines.playtime, ["enabled"], true))} onChange={(event) => updateConfig("playtime", { enabled: event.target.checked })} /> Enabled</label><label><input type="checkbox" checked={Boolean(getValue(engines.playtime, ["allowManualEntries"], true))} onChange={(event) => updateConfig("playtime", { allowManualEntries: event.target.checked })} /> Manual entries</label><select style={input} value={getValue(engines.playtime, ["defaultSource"], "manual")} onChange={(event) => updateConfig("playtime", { defaultSource: event.target.value })}>{SOURCES.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}</select><input style={input} type="number" placeholder="Max hours per record" value={getValue(engines.playtime, ["maxHoursPerRecord"], 50000)} onChange={(event) => updateConfig("playtime", { maxHoursPerRecord: Number(event.target.value || 0) })} /></div><div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}><button type="button" style={button} disabled={saving} onClick={() => void saveConfig("playtime", engines.playtime.config || {}, "Saved playtime engine settings.")}>Save Playtime Engine</button></div><div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(260px,1fr))", gap: 12, marginTop: 12 }}><div style={card}><div style={{ fontWeight: 800, marginBottom: 8 }}>Record Hours</div><div style={{ display: "grid", gap: 8 }}><input style={input} placeholder="Discord user ID" value={playtimeForm.userId} onChange={(event) => setPlaytimeForm((prev) => ({ ...prev, userId: event.target.value }))} /><input style={input} placeholder="Game title" value={playtimeForm.game} onChange={(event) => setPlaytimeForm((prev) => ({ ...prev, game: event.target.value }))} /><select style={input} value={playtimeForm.platform} onChange={(event) => setPlaytimeForm((prev) => ({ ...prev, platform: event.target.value }))}>{PLATFORMS.map((entry) => <option key={entry.value || "any"} value={entry.value}>{entry.label}</option>)}</select><input style={input} type="number" placeholder="Hours" value={playtimeForm.hours} onChange={(event) => setPlaytimeForm((prev) => ({ ...prev, hours: event.target.value }))} /><select style={input} value={playtimeForm.sourceKey} onChange={(event) => setPlaytimeForm((prev) => ({ ...prev, sourceKey: event.target.value }))}>{SOURCES.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}</select><input style={input} placeholder="Notes" value={playtimeForm.notes} onChange={(event) => setPlaytimeForm((prev) => ({ ...prev, notes: event.target.value }))} /></div><div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}><button type="button" style={button} disabled={saving} onClick={() => void runAction("playtime", "recordHours", { ...playtimeForm, hours: Number(playtimeForm.hours || 0) }, "Playtime record saved.")}>Record Hours</button></div></div><div style={card}><div style={{ fontWeight: 800, marginBottom: 8 }}>Top Recorded Hours</div><div style={{ display: "grid", gap: 8 }}>{playtimeLeaders.length ? playtimeLeaders.map((entry) => <div key={`${entry.userId}-${entry.rank}`}><div style={{ fontWeight: 700 }}>#{entry.rank} {entry.name}</div><div style={{ color: "#ffb3b3", fontSize: 12 }}>{entry.value}</div></div>) : <div style={{ color: "#ffb3b3" }}>No recorded hours yet.</div>}</div></div></div><div style={{ marginTop: 12, display: "grid", gap: 8 }}>{playtimeRecords.length ? playtimeRecords.map((entry) => <div key={entry.id} style={{ ...card, marginBottom: 0 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}><div><div style={{ fontWeight: 700 }}>{entry.title}</div><div style={{ color: "#ffb3b3", fontSize: 12 }}>{entry.value}</div></div><button type="button" style={button} disabled={saving} onClick={() => void runAction("playtime", "removeRecord", { recordId: entry.id }, "Playtime record removed.")}>Remove</button></div></div>) : <div style={{ color: "#ffb3b3" }}>No playtime records yet.</div>}</div></div>
